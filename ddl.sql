...
create table if not exists public.session_price (
    id serial,
    session_id int4 not null,
    seat_type_id int4 not null,
    price int not null,
    CONSTRAINT session_price_pkey PRIMARY KEY (id),
    unique (session_id, seat_type_id),
    CONSTRAINT fk_3 FOREIGN KEY (session_id) REFERENCES session(id) ON DELETE restrict,
    CONSTRAINT fk_4 FOREIGN KEY (seat_type_id) REFERENCES seat_type(id) ON DELETE restrict
);
...
create table if not exists public.order (
    ...
    CONSTRAINT fk_10 FOREIGN KEY (session_id) REFERENCES session(id) ON DELETE restrict
);
...
create table if not exists public.order_item (
    ...
    CONSTRAINT fk_7 FOREIGN KEY (seat_id) REFERENCES seat(id) ON DELETE restrict,
