...
begin
	...
	insert into movies."types" ("name", data_type) values ('Число', 'int4') returning id into numId;
	insert into movies."types" ("name", data_type) values ('Важные даты', 'date') returning id into serviceDateId;
	insert into movies."types" ("name", data_type) values ('Служебные даты', 'date') returning id into scheduledDateId;



	insert into movies."attributes" ("name", type_id) values ('Мировая премьера', serviceDateId) returning id into worldPremierId;
	insert into movies."attributes" ("name", type_id) values ('Премьера в РФ', serviceDateId) returning id into ruPremierId;	
	insert into movies."attributes" ("name", type_id) values ('Начало продаж', scheduledDateId) returning id into startSalesId;	
	insert into movies."attributes" ("name", type_id) values ('Начало рекламной компании', scheduledDateId) returning id into startReclamId;	
	insert into movies."attributes" ("name", type_id) values ('Конец рекламной компании', scheduledDateId) returning id into endReclamId;	
	...
	insert into movies."attributes" ("name", type_id) values ('Рейтинг', numId) returning id into ratingId;	
	insert into movies."attributes" ("name", type_id) values ('Длительность', numId) returning id into durationId;

--	
----	Побег из Шоушенка
	insert into movies.films (title) values ('Побег из Шоушенка') returning id into filmId;

	insert into movies."values" (film_id, attr_id, val_text) values 
			(filmId, premiaId, 'Oscar'),
			(filmId, premiaId, 'Венецианский фестиваль'),
			(filmId, sloganId, 'Страх - это кандалы. Надежда - это свобода');

	insert into movies."values" (film_id, attr_id, val_date) values 
			(filmId, startSalesId, (now() + interval '20 days')::date),
			(filmId, startReclamId, current_date),
			(filmId, ruPremierId, '2019-12-12'),
			(filmId, worldPremierId, '2019-10-24');
	
	insert into movies."values" (film_id, attr_id, val_num) values 
			(filmId, ratingId, 16),
...
----	Бойцовский клуь
	...
	insert into movies."values" (film_id, attr_id, val_text) values 
			(filmId, countryId, 'США'),
			(filmId, recensId, 'Лавочка, с помощью которой Рассказчик пытался разбить стекло, исчезает, когда он входит в здание на Franklin str.'),
			(filmId, sloganId, 'Интриги. Хаос. Мыло');

	insert into movies."values" (film_id, attr_id, val_date) values 
			(filmId, startReclamId, (now() + interval '20 days')::date),
