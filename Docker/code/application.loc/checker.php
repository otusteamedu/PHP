...
function checkMemcached(): bool
{
    ...
    $connection = $memcache->addServer($serverMemcached, 11211);
    ...
    $key = "key";
    $value = "This is Test value for MemCached1";
    $memcache->set($key, $value);
    $get_value = $memcache->get($key);
    if ($value == $get_value) {
        showSuccess("Connection to server `$serverMemcached` SUCCESSED!", $tag);
        return true;
    } else {
        showError("Connection to server `$serverMemcached` FAILED!", $tag);
    }
    return false;
}
