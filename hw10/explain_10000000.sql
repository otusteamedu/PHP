...
---------------------------------------------------------------------
    ...
    -- В таблице сессий - 60 000 строк. Сейчас используется последовательное сканирование по таблице сессий (Seq Scan on sessions), 
    -- с условием по дате. В результате отсеяно 59901, найдено 99 строк, время на их получение - 79 ms.
    ...
    -- Теперь используется сканирование индекса (Bitmap Index Scan on idx_session_date).
    -- Выбрано 99 строк, время на их получение - 0.051 ms.
    -- Далее планировщик проверяет фактическое наличие в таблице нужных строк (Bitmap Heap Scan on sessions),
    -- основное время запроса уходит на это (0.354 ms).
    -- Как итог - время выполнения значительно снизилось, cost снизился.
...
---------------------------------------------------------------------
    ...
    -- Планировщик уже использует индексы, добавленные при создании таблиц, в частности idx_movie_prop_values_float.
    -- Однако, для индекса idx_movie_prop_values_float применяется фильтрация по свойству (Filter: (property = 12)).
    -- Основное время запроса уходит на это (5107.101 ms), в результате отсеивается 2 682 020 строк. Всего же в таблице 3 010 000 строк.
    -- Только после этого планировщик отсеивает нужные 100 строк.
    -- Добавим ID свойства в индекс, чтобы планировщик сразу знал с какими строками работать.
    ...
    -- Теперь используется обновленный индекс, планировщик сразу использует строки с нужным свойством, и выбирает 100 из них (0.218 ms).
    -- Время выполнения снизилось, cost снизился.
...
---------------------------------------------------------------------
    ...
    -- Планировщик использует параллельное сканирование.
    -- Используя индекс idx_movie_prop_values_float находит все строки со свойством 1,
    -- и выбирает из самой таблицы movie_property_values те у которых boolean_value = true.
    -- Добавим отдельный индекс по колонкам property и boolean_value.
    ...
    -- Теперь планировщик использует новый индекс idx_boolean_prop, 
    -- сразу выбирая нужные строки по условию Index Cond: ((property = 1) AND (boolean_value = true)).
    -- Затем в основной таблице перепроверяет их актуальность.
    -- В результате время выполнения снизилось, cost снизился.
...
---------------------------------------------------------------------
    ...
    -- Запрос уже использует индексы, добавленные при создании таблиц, в частности basket_unique.
    -- Для объединения таблиц используется несколько вложенных циклов.
    -- При этом планировщик последовательно сканирует таблицы seats и halls.
    -- Добавление индексов на эти таблицы положительного эффекта не дало.
    -- Попробуем переписать запрос, например с использованием подзапроса.
    ...
    -- Время выполнения незначительно уменьшилось, cost незначительно уменьшился.
...
---------------------------------------------------------------------
    ...
    -- Планировщик использует параллельное выполнение.
