...
FROM films
    ...
    WHERE fa.type_id='3' AND service_dates=current_date
) today_tasks ON films.id = today_tasks.film_id
    LEFT JOIN (
    SELECT film_id, fa.name AS task FROM film_attributes_values fav
    LEFT JOIN film_attributes fa ON fav.attribute_id = fa.id
    WHERE fa.type_id='3' AND service_dates=current_date + interval '20 days'
...
AS SELECT f.name AS film_name,
          ...
          case fa.type_id
              ...
              WHEN 2 THEN cast(fav.review as text)
              WHEN 3 THEN cast(fav.important_dates as text)
              WHEN 4 THEN cast(fav.price as text)
              ELSE NULL
              end AS value
   FROM films f
