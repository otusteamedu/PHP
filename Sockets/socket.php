...
final class socket
{
    ...
    {
        $this->socket = socket_create($domain = $this->domain, $type = $this->type, $protocol = $this->protocol);
     ...
     * для одно-пользовательской модели
     *
     * @param \Socket|null $acceptedSocket
     */
    public function closeAcceptedSocket(\Socket $acceptedSocket = null): void
    {
     ...
     * @throws Exception
     */
    public function bind(string $address = '127.0.0.1'): void
    {
    ...
    /**
     * Устанавливает прослушку для серверного сокета $this->socket
     * @throws Exception
     */
     ...
     * @param bool $acceptBlockMode В случае, если 'true' блокирует дальнейшее действие скрипта до возникновения соединения
     * @return bool|\Socket
     * @throws Exception
     */
    public function accept(bool $acceptBlockMode = true): bool|\Socket
    {
        if (!$acceptBlockMode) {
     ...
     * @throws Exception
     */
    public function select($read = null, $write = null, $except = null): ?array
    {
