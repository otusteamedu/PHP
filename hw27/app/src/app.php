...
class App
{
    ...
    private function handleHttpRequest(): void
    {
        try {
            ...
            $route = $router->match($request = $this->getRequest());
            $request->withAttributes($route->getAttributes());

            $handlerParams = array_merge(['request' => $request], $request->getAttributes());
            $handler = $this->container->get($route->getHandler()[0]);
            $response = $this->container->callMethod($handler, $route->getHandler()[1], $handlerParams);
